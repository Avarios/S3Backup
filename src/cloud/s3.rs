...
pub async fn get_all_files_bucket(bucket_name: &String) -> Result<Vec<S3File>, Box<dyn Error>> {
    ...
    println!("[TRACE]::S3::GETFILES -> Starting get files from bucket : {}",&local_bucket_name);
    ...
    match file_list {
        Ok(file_list) => {
            ...
            println!("[TRACE]::S3::GETFILES -> Get all files took {:?} for bucket {}", timer.elapsed(), &local_bucket_name);
            return Ok(file_vec);
        }
        Err(e) => {
            return Err(format!(
...
pub async fn put_file(path: &String, bucket: &str) -> Result<(), Box<dyn Error>> {
    println!("[TRACE]::S3::PUT_FILES -> Starting to read file stream for file : {}",&path);
    let timer = Instant::now();
    ...
    println!("[TRACE]::S3::PUT_FILES -> Filestream read took {:?} for file {}", timer.elapsed(), &path);
    ...
    let put_object_output = s3client
        ...
        .send()
        .await;
    println!("[TRACE]::S3::PUT_FILES -> Upload took {:?} for file {}", timer.elapsed(), &path);
